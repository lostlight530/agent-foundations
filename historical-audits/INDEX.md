# Historical Independent Audit Archive

This directory archives historical External Independent GPT audit, correction, evidence-accounting, reconciliation, and maintenance records for Agent Foundations. Archived bodies are moved with their original Git blobs; archive placement does not rewrite their historical observations or verification state.

## Scope boundaries

Not archived here:
- Jules Daily Research, Weekly Cascade, or Monthly Strategic Blueprint task artifacts
- active FOUNDATION contracts, source registry, evidence policy, specs, plans, schemas, checkers, or workflows
- current external-maintenance procedure/templates

## Archive classes

1. `01-corrections-and-errata/` — source/provenance errata and dated corrections to prior interpretation
2. `02-document-audits/` — independent audits of canonical/public document corpora
3. `03-stage-and-period-audits/` — provisional/as-of period reconciliation
4. `04-evidence-and-closure-ledgers/` — evidence accounting and natural-period closure ledgers
5. `05-maintenance-and-reconciliation/` — cross-period Jules cadence/content reconciliation and successor maintenance records

## Relocation map

| Historical purpose | Archived path | Previous path |
| --- | --- | --- |
| August source metadata errata | `01-corrections-and-errata/2026-08-10--august-source-metadata--errata.md` | `docs/AUGUST_2026_SOURCE_ERRATA.md` |
| W33 source/provenance errata | `01-corrections-and-errata/2026-08-17--w33-source-provenance--errata.md` | `docs/AUGUST_2026_W33_ERRATA.md` |
| W34 source/provenance errata | `01-corrections-and-errata/2026-08-24--w34-source-provenance--errata.md` | `docs/AUGUST_2026_W34_ERRATA.md` |
| August 31 Daily source/claim calibration | `01-corrections-and-errata/2026-09-01--august-31-source-claim--reconciliation.md` | `docs/AUGUST_2026_31_RECONCILIATION.md` |
| Jules task/PR/merge correction | `01-corrections-and-errata/2026-09-07--jules-task-merge--correction.md` | `docs/JULES_CADENCE_2026-09-07_TASK_MERGE_CORRECTION.md` |
| Public-document five-axis audit | `02-document-audits/2026-08-28--foundation-documents--document-audit.md` | `FOUNDATION/2026-08-28-DOCUMENT-AUDIT.md` |
| August 24-27 research reconciliation | `03-stage-and-period-audits/2026-08-27--august-24-27--period-reconciliation.md` | `docs/AUGUST_2026_24_27_RECONCILIATION.md` |
| August 1-27 evidence ledger | `04-evidence-and-closure-ledgers/2026-08-27--august-01-27--evidence-ledger.md` | `docs/AUGUST_2026_01_27_EVIDENCE_LEDGER.md` |
| August 1-30 evidence ledger | `04-evidence-and-closure-ledgers/2026-08-30--august-01-30--evidence-ledger.md` | `docs/AUGUST_2026_01_30_EVIDENCE_LEDGER.md` |
| August final natural-month evidence ledger | `04-evidence-and-closure-ledgers/2026-08-31--august-01-31--evidence-ledger.md` | `docs/AUGUST_2026_01_31_EVIDENCE_LEDGER.md` |
| Cross-period Jules cadence reconciliation | `05-maintenance-and-reconciliation/2026-09-06--jules-cadence-jul-01-sep-06--reconciliation.md` | `docs/JULES_CADENCE_2026-07-01_THROUGH_2026-09-06_RECONCILIATION.md` |
| Jules content-contract reconciliation | `05-maintenance-and-reconciliation/2026-09-06--jules-content-contract-jul-01-sep-06--reconciliation.md` | `docs/JULES_CONTENT_CONTRACT_2026-07-01_THROUGH_2026-09-06_RECONCILIATION.md` |
| September ten-day successor reconciliation | `05-maintenance-and-reconciliation/2026-09-10--sep-01-10--cadence-reconciliation.md` | `docs/JULES_CADENCE_2026-09-10_TEN_DAY_SUCCESSOR_RECONCILIATION.md` |
| September 1-13 maintenance reconciliation | `05-maintenance-and-reconciliation/2026-09-13--sep-01-13--maintenance-reconciliation.md` | no live `docs/SEPTEMBER_2026_01_13_RECONCILIATION.md` on current main |

## Current recovery note

As of 2026-09-15, `05-maintenance-and-reconciliation/2026-09-13--sep-01-13--maintenance-reconciliation.md` is the latest completed independent maintenance record preserved in this archive. Its internal `CURRENT_MAINTENANCE_RECORD` label is the record's write-time role, not a live path under `docs/`.

Current repository authority remains with the active `FOUNDATION/` evidence, provenance, review, source-registry, and maintenance contracts together with current repository evidence. The archive preserves point-in-time interpretation and does not replace those current surfaces.

The old paths remain recoverable through Git history. The archive is historical storage, not current Jules task output, current repository authority, or the external-maintenance reference template.


## Successor maintenance records

- `05-maintenance-and-reconciliation/2026-09-19--sep-19--maintenance-reconciliation.md` — 2026-09-19 S49 source/state reconciliation

As of 2026-09-19 this is the latest completed independent periodic-maintenance record in the archive. Current FOUNDATION contracts, source registry and current repository evidence remain higher authority for present interpretation.
